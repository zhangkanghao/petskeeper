...
public class FollowModule extends BaseModule {
    ...
    public Object getFollower(@Param("userId") int userId) {
        /**select * from t_user_profile where uid in(select from_id from t_follow where to_id =%d)*/
        QueryResult rs = new QueryResult();
        Condition cnd = Cnd.wrap("uid in (select from_id from t_follow where to_id =${userId}) ", new NutMap().setv("userId",userId));
        rs.setList(dao.query(UserProfile.class, cnd, null));
        rs.setPager(null);
        return rs;
    }
    ...
    public Object getFollowing(@Param("userId") int userId) {
        QueryResult rs = new QueryResult();
        Condition cnd = Cnd.wrap("uid in(select to_id from t_follow where from_id=${userId})", new NutMap().setv("userId",userId));
        rs.setList(dao.query(UserProfile.class, cnd, null));
        rs.setPager(null);
        return rs;
    }
